...
- **Descompresión de archivos** (`.zip`, `.rar`, `.7z`, `.tar`, `.gz`).
- **Notificaciones UI**:
...
- `controls/`: Lógica de control por funcionalidad.
  ...
  - `Controllerdecompress.py`: Descompresión de archivos.
  - `ControllerBoot.py`: Creación de carpetas iniciales.
  - `ControllerSettings.py`: Acceso/config de base de datos.
  - `ControllerConvertFile.py`: Conversión PDF <-> Word (opcional, no expuesto en UI).
...
## Licencia
Indica aquí la licencia del proyecto (p. ej., MIT) si aplica.
